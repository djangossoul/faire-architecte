	...
	window.ScrollEffects = function() {
		...
		if (document.body.classList.contains("smooth-scroll"))  {
			...
			class EdgeEasingPlugin extends Scrollbar.ScrollbarPlugin {
				...
				transformDelta(delta) {
					...
					// clamps momentum within [-offset, limit - offset] — x locked to 0 to prevent horizontal scroll
					this.scrollbar.setMomentum(0, Math.max(-offset.y, Math.min(y, limit.y - offset.y)));
					return { x: 0, y: 0 };
				}
		...
		gsap.set('.header-gradient', {
			...
			var bgColor = document.getElementById('clapat-page-content').getAttribute('data-bgcolor');
				//document.querySelector('.header-gradient').style.maskImage = `linear-gradient(${bgColor} 75%, transparent 100%)`;
				//document.querySelector('.header-gradient').style.webkitMaskImage = `linear-gradient(${bgColor} 75%, transparent 100%)`;
				document.querySelector('.header-gradient').style.maskImage = `linear-gradient(rgba(255, 255, 255, 1), rgba(255, 255, 255, 0.616), rgba(255, 255, 255, 0.384), transparent )`;
				document.querySelector('.header-gradient').style.webkitMaskImage = `linear-gradient(rgba(255, 255, 255, 1), rgba(255, 255, 255, 0.616), rgba(255, 255, 255, 0.384),transparent )`;
			}
		});
